...
export async function PUT(request: Request) {
    try {
        ...
        if (error) {
            return NextResponse.json({
                success: false,
                message: "Invalid saving data", error
            }, { status: 400 })
        }
        const query = 'UPDATE savings SET name = $1, date = $2, amount = $3, goal = $4, status = $5, description = $6 WHERE id = $7 RETURNING *'
        const result = await pool.query(query, [body.name, body.date, body.amount, body.goal, body.status, body.description, body.id])
        return NextResponse.json({
            success: true,
            data: result.rows[0]
        }, { status: 200 })
...
export async function PATCH(request: Request) {
    try {
        ...
        const query = 'UPDATE savings SET status = $1, updatedAt = NOW() WHERE id = $2 RETURNING *'
